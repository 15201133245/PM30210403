...
void FormatAll(void)
{
	...
	pAt45 = &at45[0];
	for(i = PROTECTED_START; i < 4096; i++)
	{
		SPID_ConfigureCS(pAt45->pSpid, 0, SPI_CSR(MAIN_MCK, AT45_SPI_MCK));
		do
		{
...
#if POS_TYPE==0
		pAt45 = &at45[CONFIG_ID];
		for(i = CASHPOS_TRANS_ADR; i < (CASHPOS_TRANS_ADR + 8); ++i)
		{
			SPID_ConfigureCS(pAt45->pSpid, CONFIG_ID, SPI_CSR(MAIN_MCK, AT45_SPI_MCK));
            do
			{
...
unsigned char LoadGradePriceList(void)
{
	...
	{
		memcpy((unsigned char *)(&GradePrice),(unsigned char *)(&GradeTmp),sizeof(GradePrice));
		for(i = 0; i < GradePrice.FieldNum; ++i)//20130815 liuzg for download ppu twice, dutyOff
		{
		...
		return 0;//20130815 liuzg for download ppu twice, dutyOff
	}
	else
	{
		memcpy((unsigned char *)(&GradePrice),(unsigned char *)(&GradeTmp),sizeof(GradePrice));
...
{
	memcpy(s,WhiteListHead.InuredDate,14);
	return(0);
}
